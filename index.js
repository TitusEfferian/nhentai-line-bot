...
const handleEvent = (event) => {
  ...
  if (event.message.type === "text") {
    ...
    if (event.message.text.toLowerCase().startsWith("!efferian")) {
      const userEfferianMoment = event.message.text
        .toLowerCase()
        .split("!efferian ")[1]
        .split("@")[1];
    ...
    if (event.message.text.toLowerCase().startsWith("!top")) {
      const callBackReturnWithData = (topPlayerData) =>
      ...
      const callBackReturnNoData = () =>
        client.replyMessage(event.replyToken, {
          type: "text",
          text: "belum ada data di leaderboard",
        });
      const callBackReturnCatchError = (err) =>
        client.replyMessage(event.replyToken, {
          type: "text",
          text: "error server: " + JSON.stringify(err),
        });
      handleGetTopPlayer(
        callBackReturnWithData,
        callBackReturnNoData,
        callBackReturnCatchError
      );
    }
    if (event.message.text.toLowerCase().startsWith("g/")) {
      (async () => {
        try {
          ...
          const resultFetchBeforeParse = await fetch(
            `${nhentaiCrawlerV2}?nhentai_id=${nhentaiCode}`
          );
          const {
            arrayOfImage,
            num_pages,
            success,
            arrayOfCharacter,
          } = await resultFetchBeforeParse.json();
          ...
          for (let a = 1; a <= numberOfColumns(); a++) {
            arrayOfColumns.push({
              imageUrl: `${nhentaiByPassOriginal}?source=${
                arrayOfImage[a - 1].preview
              }`,
              action: {
                type: "uri",
                label: handleLabel(a),
                uri: `https://nhentai-line-bot.vercel.app/${nhentaiCode}`,
    ...
    if (event.message.text.toLowerCase().startsWith("nhentai")) {
      const searchKeywords = event.message.text
        .toLowerCase()
        .split("nhentai ")[1];
    ...
    if (event.message.text.toLowerCase().startsWith("!nhentaiinfo")) {
      (async () => {
        const nhentaiCode = event.message.text
          .toLowerCase()
          .split("!nhentaiinfo ")[1];
